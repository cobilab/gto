...
\item \texttt{gto\char`_fastq\char`_variation\char`_map}: it ... to do 

\item \texttt{gto\char`_fastq\char`_variation\char`_filter}: it ... to do 

\item \texttt{gto\char`_fastq\char`_variation\char`_visual}: it ... to do 

\item \texttt{gto\char`_fastq\char`_falcon}: it measures similarity between any FASTQ file, independently from the size, against any multi-FASTA database.
...
\input{\FASTQToolsPath/Falcon.tex}
